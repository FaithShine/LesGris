	...
	if (!(appName in vendor)) {
		...
		vendor[appName].initializeTable = (function initializeTable(table, callback) {
			...
			var $table = (table instanceof $ ? table : $(table)),
				...
				columForSort = $table.data('sort-column');

			/* costum sort */
			var getDataValue = function(index) {
			...
			var getAlg = function(dataType, dataValue) {
				var sortAlg = 'natural-nohtml';
				...
				switch(dataType) {
					...
					case 'number':
						...
						break;
					case 'text':
						if(vendor[appName].isNumber(dataValue)) {
							sortAlg = 'number-html';
						}
						break;
				}
			...
			$table.find('thead')
				...
				.find('th')
				.each(function(index) {
					var type = $(this).attr('data-cell-format-type');
					var value = getDataValue($(this).index());
			
					algs.push({
						"sType": getAlg(type ? type : 'text', value)
					});
				});
			
			jQuery.extend( jQuery.fn.dataTableExt.oSort, {
				"number-html-pre": function ( a ) {
					var x = String(a);
					x = numeral(x).value();
				...
				"number-html-asc": function ( a, b ) {
					return ((a < b) ? -1 : ((a > b) ? 1 : 0));
				},

				"number-html-desc": function ( a, b ) {
					return ((a < b) ? 1 : ((a > b) ? -1 : 0));
				},
			});
