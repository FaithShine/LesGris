...
class SupsysticTables_Overview_Controller extends SupsysticTables_Core_BaseController
{
    ...
    protected function getServerSettings() {
		...
		$settings = array(
			...
			'MySQL version' => array('value' => $this->translate('Undefined')),
			'MySQLi driver' => array('value' => $wpdb->use_mysqli ? 'Yes' : 'No'),
			'PHP Safe Mode' => array('value' => 'No'),
		...
		if(version_compare($phpVersion, '5.4.0', '<=')) {
			...
			if($save_mode) {
				$settings['PHP Safe Mode'] = array('value' => 'Yes', 'error' => $save_mode);
			}
		}
		if(version_compare($phpVersion, '7.0.0', '<=')) {
			if(function_exists('mysql_get_server_info')) {
				$settings['MySQL version']['value'] = @mysql_get_server_info();
			}
		} else {
			if(function_exists('mysqli_get_server_info') && function_exists('mysql_connect')) {
				$settings['MySQL version']['value'] = @mysqli_get_server_info(@mysql_connect(DB_HOST, DB_USER, DB_PASSWORD));
			}
		}
