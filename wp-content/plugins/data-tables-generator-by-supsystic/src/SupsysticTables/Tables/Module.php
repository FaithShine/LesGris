...
class SupsysticTables_Tables_Module extends SupsysticTables_Core_BaseModule
{
    ...
    public function render($id)
    {
        ...
        if (!$this->single && file_exists($cachePath) && $this->getEnvironment()->isProd()) {
        ...
        do_action('supsystic_render_table', $id, $table);
		...
		if($this->single) {
			...
			foreach($table->rows as $key => $row) {
				if ($this->single['row'] === $key + 1) {
					foreach($row['cells'] as $index => $cell) {
						if($index == $this->single['col'] - 1) {
							// For correct work of saving data through editable fields
							$table->rows[$key]['cells'][$index]['row'] = $this->single['row'];
							$table->rows[$key]['cells'][$index]['col'] = $this->single['col'] - 1;
        ...
        if (!$this->single && isset($this->cacheDirectory)) {
	...
	{
		$this->single = array('row' => $tableRowId, 'col' => $tableColId);
