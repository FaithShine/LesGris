(function ($, app, undefined) {
	$(document).ready(function () {
		...
		$cloneDialog = $('#cloneDialog').dialog({
			...
			modal:    true,
			open: function() {
				$(this).next().find('button:last-of-type').html('OK').show();
			},
			buttons:  {
				Close: function () {
					$(this).dialog('close');
		...
		function saveTable(preview) {
			...
			$.each(editor.getData(), function (x, rows) {
				...
				$.each(rows, function (y, cell) {
					...
					// Set cell format
					rowsData.type = meta.type ? meta.type : 'text';
					rowsData.formatType = meta.formatType ? meta.formatType : '';

					switch(rowsData.formatType) {
						case 'currency':
							rowsData.format = formData.find('[name="currencyFormat"]').val();
							break;
						case 'percent':
							rowsData.format = formData.find('[name="percentFormat"]').val();
							break;
						case 'date':
							rowsData.format = formData.find('[name="dateFormat"]').val();

							var date = moment(rowsData.data, rowsData.format);

							if (date.isValid()) {
								rowsData.dateOrder = date.format('x');
							}
							break;
						default:
							rowsData.format = meta.format;
							break;
					}
					...
					if (app.Models.Tables.isFormula(cell)) {
						...
						if (value !== undefined) {
							...
							rowsData.calculatedValue = value;
						}
					}
					...
					rowsData.meta = classes;

					// Set comments for cell
					if ('comment' in meta && meta.comment.length) {
						rowsData.comment = meta.comment;
					}

					// Set column width by cells of first row
					if (x == 0) {
						columnsWidth.push(editor.getColWidth(y));
					}

					row.cells.push(rowsData);
				});
